...
bullet.damage(20)
###########################################################
heal = Actor.Heal(pygame) #힐팩 액터
heal.setImage("hill.jpg")
heal.setScale(40, 40)
heal.estimateCenter()

PowerUp = Actor.PowerUp(pygame)#파워업 액터
PowerUp.setImage("")
PowerUp.setScale(40,40)
...
while not done:  # done이 False를 유지하는 동안 계속 실행, not False = Ture
    ...
    if heal.y > screen.get_width():  # 힐팩이 너무 밑으로 내려가면
        heal.islive = False
    ...
    if heal.islive:
        ...
        heal.estimateCenter()
        if heal.isCollide(hero):  # 히어로가 쳐먹은 경우
            heal.islive = False
            hero.increaseVitality(20) # 생명력 증가량

    if time % PowerUp.interval < 0.1 and PowerUp.islive == False:
        PowerUp.reset(screen)

    if PowerUp.y > screen.get_width():  # 힐팩이 너무 밑으로 내려가면
    ...
    if PowerUp.islive:
        ...
        PowerUp.estimateCenter()
        if PowerUp.isCollide(hero):  # 히어로가 쳐먹은 경우
            PowerUp.islive = False
            bullet.damage()+10 # 공격력 증가량
    ...
    if bulletFire == True:  # 총을 쏘고 있는가? 이게 참이라면.. 총알을 계속 이동시켜야함
        ...
        if collsion == True:
            print("부딪힘")
            enermy.decreaseVitality(bullet.damage())
            bulletFire = False
